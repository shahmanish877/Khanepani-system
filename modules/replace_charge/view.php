<h1 class="title">Meter/Parts Replace Charge</h1>

<?php 
$data = $replace_charge->getAll();
...
<a href="?page=replace_charge_form" class="btn">Add Replace Charge</a>
...
<?php 
	...
	if( isset($_SESSION) ){
		if($_SESSION['success_msg']){
			...
			echo "<h4 class='danger'> Client's replace charge successfully deleted </h4>";			
		}
	}
 ?>
...
<table border="1" width="100%" class="table" id="result">
	...
	{ ?>
		<tr >
			...
			<td> <?php echo $d['id']; ?> </td>
			<td> <a href="meter_replacecharge_print.php?print_id=<?php echo $d['id']; ?>"> <?php echo $d['name'];?> </a> </td>
			...
			<td>
				<a href="?page=replace_charge_form&edit_id=<?php echo $d['id']; ?>">
					Update
				</a> 
				|
				<a href="?page=replace_charge&del_id=<?php echo $d['id']; ?>" onclick="return confirm('Do you really want to delete?')">
					Delete
				</a> 
...
<script>
	$(document).ready(function(){
		...
		{
			$.ajax({
				url:"modules/replace_charge/search.php",
				method:"post",
				data:{query:query},
